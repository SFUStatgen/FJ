...
* Next we read into our R environment the population sequences 
that we will sample from to create the data. 
...
* To plot a pedigree, we select it from `expeds` and
coerce it to class `ped`. We hide the RV status in the pedigree 
...
* Lastly, we simulate sequence data for the pedigrees. 
...
# Summarize sequence data of the study
...
# Analyze the sequence data of the study
...
at most one founder. We therefore filter out "invalid" RVs that are not consistent with this assumption. 
...
* We rank each RV in our filtered dataset using the value of
...
* The cRV 8_23020454 from pedigree 1 had a top-10 rank from all methods.
From the pedigree diagram we see that this 
RV is in two HL-affected individuals who are second cousins.
* The cRV 8_23068413 from pedigree 2 had a top-10 rank from the LR methods
only. From the pedigree diagram we see that 
this RV is in two HL-affected individuals who are first cousins.
...
individuals and have reasonably small p-values (e.g., 0.043 and 0.017,
respectively by the global methods).
* The cRV 8_118923860 was observed in only one affected individual in 
pedigree 3 and so, not surprisingly, has large p-values (about 0.35
by the global methods) and ranks (about 0.44 by the LR methods).
...
* The second function written for this analysis is a modified
...
is to split the counts of each variant by disease subtype. 
    ...
    that there are three families in the study and that there
    are 12 affected individuals in total.
