...
MLEs of the two transmission probabilities, the binary configuration vector
which is already expressed in base-10 as the `binID`, the number of 
...
```{r}
  ...
  output <- list(statvals=as.matrix(statvals), as.matrix(pvals=pvals))
```
...
## Install R packages and call `cd_new()`
...
* Finally, we provide a function call to use for testing purposes.
...
## Bundling code as an R function

*  You can generate an  R script that contains the `cd_new()` function
by cutting-and-pasting the following into your R console:

```{r}
knitr::purl(input="cd_new.Rmd",output="cd_new.R",documentation=0L)
```

* This will produce an R script containing all the code chunks
in this `Rmd` file. 
* Alter the resulting R script as follows. Delete the code on the first line
that sets up `knitr` (i.e., the first code chunk in this `.Rmd` file) as
well as on line 252 that contains the above `purl()` command.
* You can now `source("cd_new.R")` into your R session to 
define the `cd_new()` function.
